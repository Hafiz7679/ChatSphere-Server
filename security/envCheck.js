...
function validateEnv() {
  ...
  const exitOnFailure = !isDev || process.env.VALIDATE_ENV === "strict";
  const blockedCount = exitOnFailure ? critical + high : critical;

  console.log(`\n${critical + high + medium} issues found (${critical} critical, ${high} high, ${medium} medium)`);

  if (blockedCount > 0) {
    const msg = exitOnFailure
      ? "Resolve critical and high-priority issues before deploying to production."
      : "Resolve critical issues. High/medium warnings can be deferred in development.";
    console.log(`\n⚠️  ${msg}\n`);
    return false;
  }
